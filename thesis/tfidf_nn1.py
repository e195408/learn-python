...
def create_tfidf_dict(filename_list):
    ...
    return file_tfidf_dict


def sorted_tfidf_dict(file_tfidf_dict, filename):
    print('filename:{0}'.format(filename))
    # print(file_tfidf_dict[filename])
    tfidf_sorted = sorted(file_tfidf_dict.items(),
...
def gcreate_tfidf_dict(filename_list):
    file_word_counter = Counter()
    # sentences = sentences_generator(filename_list)
    # ans = countup_words(sentences)
    file_tf_dict = {}
...
def print_tfidf_dict(file_tfidf, filename):
    print('filename:{0}'.format(filename))
    # print(file_tfidf_dict[filename])
    tfidf_sorted = sorted(file_tfidf.items(),
                          key=lambda x: x[1], reverse=True)
    print('word number:{0}'.format(len(tfidf_sorted)))
    print('number\tword\t\tscore')
    for i in range(0, 10):
...
def get_namelab_from_path(filename_path):
    ...
    filename = filename_path[idx1+len(target1):idx2]
    # print(filename)
...
def main():
    ...
    file_tfidf_dict = {}
    file_tfidf_dict = create_tfidf_dict(filename_list)
    file_vector_dict = {}
    for filename in filename_list:
        file_tfidf = file_tfidf_dict[filename]
        print_tfidf_dict(file_tfidf, filename)
    ...
    with open('thesis/thesis_vector.csv', 'w', encoding='utf-8') as f:
        ...
        for filename_path in filename_list:
            lab, filename_plane = get_namelab_from_path(filename_path)
            print('lab:{0}'.format(lab))
            print('filename:{0}'.format(filename_plane))

            vector = file_vector_dict[filename_path]
