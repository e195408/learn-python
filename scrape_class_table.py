...
def main():
    ...
    class_list = list(p_temp.glob('*.csv'))
    # print(class_list)
    for classes_csv_path in class_list:
        file_name = os.path.splitext(os.path.basename(classes_csv_path))[0]
        with open('text/' + file_name + '.csv', 'w', encoding='utf-8') as output_file:
            ...
            with open(classes_csv_path, 'r', encoding='utf-8') as f:
                reader = csv.reader(f)
                i = 0
                for class_detail in reader:
                    if i==0:
                        i+=1
                        continue
                    print(class_detail)
                    name = remove(class_detail[0])
                    link = class_detail[1]
